...
# Add figure title
fig.suptitle('Distribution of rooms and icons for COLORFUL dataset (instances)')
...
for i, file in enumerate(files):
    ...
    ax[i, 0].hist(rooms, bins=nr_rooms, range=(0, nr_rooms), align='left', rwidth=0.8)

    # Add the count of each room above the bars with a 45 degree rotation
    for j in range(nr_rooms):
        ax[i, 0].text(j, ax[i, 0].patches[j].get_height(), f'{int(ax[i, 0].patches[j].get_height())}', ha='center', va='bottom')

    # Create histogram of icons
    ax[i, 1].hist(icons, bins=nr_icons, range=(0, nr_icons), align='left', rwidth=0.8)
    for j in range(nr_icons):
        ax[i, 1].text(j, ax[i, 1].patches[j].get_height(), f'{int(ax[i, 1].patches[j].get_height())}', ha='center', va='bottom')
