...
### PLOT ###
fig_rooms, ax_rooms = plt.subplots(1, len(class_structure["Rooms"]["labels"])+1, figsize=(24, 3), sharey=True)
fig_icons, ax_icons = plt.subplots(1, len(class_structure["Icons"]["labels"])+1, figsize=(24, 3), sharey=True)
    ...
    for j, (split, split_data) in enumerate(domain_data.items()):
        ...
        for class_type, class_data in class_structure.items():
            ...
            for k, class_name in enumerate(class_data["labels"]):
                split_dict[split][class_type][class_name][0] /= total * 2 if k == 0 else total # Background / No Icon is halved for display
    ...
    for k, class_name in enumerate(class_data["labels"] + [class_type]):
        if class_name == class_type:
            ...
            for l, (domain, split_dict) in enumerate(domain_dict.items()):
                ...
                subplots[class_type][1][k].text(l, subplots[class_type][1][k].get_ybound()[1] + 0.01, f'{samp}', ha='center', va='bottom', fontsize=fontsize/2)
        ...
        # Add the samp above the boxplot
        subplots[class_type][1][k].text(len(structure), subplots[class_type][1][k].get_ybound()[1] + 0.01, f'{samp}', ha='center', va='bottom', fontsize=fontsize/2, fontweight='bold')
        ...
        for l, (domain, split_dict) in enumerate(domain_dict.items()):
            ...
            # Set the labels
            subplots[class_type][1][k].set_xticklabels(list(structure.keys()) + ["T"], fontsize=fontsize)
...
for i, (domain, domain_data) in enumerate(structure.items()):
    for j, (class_type, class_data) in enumerate(class_structure.items()):
        ...
        # Add the samp above the boxplot
        subplots[class_type][1][-1].text(i, subplots[class_type][1][-1].get_ybound()[1] + 0.01, f'{samp}', ha='center', va='bottom', fontsize=fontsize/2)
