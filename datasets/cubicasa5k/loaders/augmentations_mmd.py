...
from datasets.cubicasa5k.loaders import svg_utils
import cv2
...
class RandomRotationsMMD(object):
...
class DictToTensorMMD(object):
    ...
    def cubi(self, sample):
        ...
        return {'image': image, 'label': label, 'target': target}
...
class RandomCropToSizeTorchMMD(object):
    ...
    def augment_dict(self, sample):
        ...
        new_target = torch.full([target.shape[0], new_h, new_w], 255)
        new_target[:, pad_h:img_h+pad_h, pad_w:img_w+pad_w] = target
...
class ResizePaddedTorchMMD(object):
    ...
    def __call__(self, sample):
        ...
        target, _, _, _ = self.resize_padded(sample['target'], self.size, fill_cval=self.fill_cval, image=True, mode='bilinear', aling_corners=False)
    ...
    def augment_dict(self, sample):
        ...
        return {'image': image, 'label': label, 'heatmaps': heatmap_points, 'scale': scale, 'target': sample['target']}
    ...
    def resize_padded(self, img, new_shape, image=False, fill_cval=0, mode='nearest',
        ...
        #interm_shape = [interm_shape[0], interm_shape[1]] # OLD!!
        interm_shape = (int(interm_shape[0]), int(interm_shape[1]))
