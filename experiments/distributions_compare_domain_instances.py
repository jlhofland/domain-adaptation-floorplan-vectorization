...
fig, ax = plt.subplots(4, 3, figsize=(24, 12), gridspec_kw={'hspace': 0.7, 'wspace': 0.1})

# Add figure title
fig.suptitle("Source vs target domain distribution (instances)")
...
for i, type in enumerate(files):
    for j, file in enumerate(type):
        ...
        ax[i, j].hist(rooms, bins=nr_rooms, range=(0, nr_rooms), align='left', rwidth=0.8)
        for k in range(nr_rooms):
            ax[i, j].text(k, ax[i, j].patches[k].get_height(), f'{int(ax[i, j].patches[k].get_height())}', ha='center', va='bottom')
        ...
        ax[i+2, j].hist(icons, bins=nr_icons, range=(0, nr_icons), align='left', rwidth=0.8)
        for k in range(nr_icons):
            ax[i+2, j].text(k, ax[i+2, j].patches[k].get_height(), f'{int(ax[i+2, j].patches[k].get_height())}', ha='center', va='bottom')
