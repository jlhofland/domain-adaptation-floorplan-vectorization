...
class CubiCasa21M(nn.Module):
    def __init__(self, classes, extract_at_256=False, latent_transformation=None):
        ...
        self.r53_a = Residual(256, 256) # Output: (B, 256, H/128, W/128)
        self.r54_a = Residual(256, 512)
        self.r55_a = Residual(512, 512) # Output: (B, 512, H/128, W/128)
    ...
    def forward(self, x, return_latent=False, return_output=True, maxpool=False):
        ...
        out5x = self.r53_a(out5a) # Output: (B, 256, H/128, W/128)

        out5a = self.r54_a(out5x)
        ...
        outlt = out5x if self.extract_at_256 else out5a
        ...
        # Return the output
        if return_latent: return out, outlt
